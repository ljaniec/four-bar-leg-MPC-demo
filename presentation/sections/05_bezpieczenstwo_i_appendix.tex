...
\begin{tikzpicture}[node distance=0.45cm and 0.42cm,>=Latex]
  ...
  \draw[->,thick] (robot.south) .. controls +(0,-1.9) and +(0,-1.9) .. (sensors.south);
\end{tikzpicture}
\end{center}
\vspace{0.2em}
\begin{itemize}
...
\end{itemize}
\vspace{-0.25em}
\begin{block}{Pełniejsza formulacja obecnej regulacji do punktu}
\[
\min \sum_k \left(
\|p_F-p_F^\star\|^2 + \|q-q_{\mathrm{nom}}\|^2 + \|\tau\|^2\right),
\qquad p_F^\star=\mathrm{const}.
\]
\[
\Phi(q,z)=0,\quad q_{\min}\le q\le q_{\max},\quad |\tau|\le\tau_{\max},\quad h(q,\dot q)\ge0.
\]
\end{block}
\begin{alertblock}{Rozszerzenia są osobnymi zadaniami}
Śledzenie trajektorii wymaga $p_F^{\mathrm{ref}}(k)$, a śledzenie ścieżki wymaga krzywej $p_F^{\mathrm{path}}(s)$ i zmiennej postępu $s$.
\end{alertblock}
\note{To jest slajd kończący i jednocześnie źródła. Symbol $p_F^\star$ jednoznacznie oznacza stały punkt zadany. Dyad jest świadomie pominięty ze względu na komercyjną licencję, a może być wspomniany ustnie.}
\end{frame}
