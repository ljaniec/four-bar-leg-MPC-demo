...
\begin{frame}[plain]
  ...
  \vspace{-1.2em}
  \begin{center}
  \small Dla odbiorców inżynierskich: jasno, ale bez ukrywania modelu, ograniczeń i ryzyk.
  \end{center}
...
\begin{tikzpicture}[scale=0.88,>=Latex]
  ...
  \draw[rounded corners=2pt,fill=softgray] (-1.55,-2.2) rectangle (1.55,-1.7);
  \node[font=\scriptsize,align=center] at (0,-1.95) {RL proponuje ruch\\MPC i CBF kontrolują ryzyko};
\end{tikzpicture}
\end{center}
\end{columns}
\source{Opis platformy z załączonego streszczenia KKR: cztery kończyny, 3 stopnie swobody na kończynę, ROS 2, CAN, tryby sterowania napędów.}
...
\begin{tikzpicture}[scale=0.72]
  \node[font=\scriptsize] at (0,2.25) {widok z góry};
  \draw[thick,rounded corners=2pt,fill=blue!7] (-1.4,1.35) rectangle (1.4,1.95);
  \foreach \x in {-1.1,1.1} {\foreach \y in {1.25,2.05} {\draw[thick] (\x,\y)--(\x+0.25,\y+0.22);}}

  \node[font=\scriptsize] at (0,0.75) {widok z przodu};
  \draw[thick,fill=blue!7] (-1.2,0.1) rectangle (1.2,0.55);
  \draw[thick] (-0.85,0.1)--(-1.25,-0.85)--(-1.55,-1.35);
  \draw[thick] (0.85,0.1)--(1.25,-0.85)--(1.55,-1.35);

  \node[font=\scriptsize] at (0,-1.75) {widok z boku};
  \draw[thick,fill=blue!7] (-1.4,-2.55) rectangle (1.4,-2.15);
  \draw[thick] (-0.95,-2.55)--(-0.55,-3.15)--(-0.35,-3.55);
  \draw[thick] (0.95,-2.55)--(0.55,-3.15)--(0.35,-3.55);
\end{tikzpicture}
\end{center}
\end{columns}
