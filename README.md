...
A small, reproducible Python package that demonstrates constrained **Model Predictive Control (MPC)** for point-to-point Cartesian foot set-point regulation on one leg of a quadruped robot with a closed four-bar mechanism.

## Terminology and exact scope

The implemented example is **not trajectory tracking** and **not path following**.

- The controller receives one constant Cartesian foot set-point `foot_setpoint`.
- That set-point is computed once before the closed-loop simulation and remains unchanged.
- The point sequences shown in plots and animations are predicted or executed motion generated by MPC; they are not desired reference paths.
- A trajectory-tracking extension would require a time-indexed reference such as `foot_reference[k]`.
- A path-following extension would require a geometric curve plus a progress variable that is not fixed by time.

The most precise description of the current example is **constrained Cartesian foot set-point regulation**, or less formally, **point-to-point foot motion using kinematic MPC**.
...
The main result figure is written as:

```text
artifacts/mpc_foot_setpoint_regulation.png
```
...
- time-indexed Cartesian trajectory tracking;
